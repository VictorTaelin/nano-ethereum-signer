...
//combines lib_utils_a, lib_utils_b, lib_utils_c, lib_bytes. replaces lib_assert
const lib_utils = lib(() => {
    "use strict";
    ...
    const join32 = (msg, start, end, endian) => {
      const len = end - start;
      assert.deepStrictEqual( (len % 4), 0, new Error('Assertion failed')); // var assert = lib_assert(); assert(len % 4 === 0);
...
BlockHash.prototype.digest = function digest(enc) {
  ...
  assert.deepStrictEqual( this.pending === null, true, new Error('Assertion failed')); // assert(this.pending === null);
...
BaseCurve.prototype._fixedNafMul = function _fixedNafMul(p, k) {
  assert.deepStrictEqual(Boolean(p.precomputed), true, new Error('Assertion failed')); // assert(p.precomputed);
...
function PresetCurve(options) {
  ...
  assert.deepStrictEqual(this.g.validate(), true, new Error('Invalid curve')); // assert(this.g.validate(), 'Invalid curve');
  assert.deepStrictEqual(this.g.mul(this.n).isInfinity(), true, new Error('Invalid curve, G*N != O')); // assert(this.g.mul(this.n).isInfinity(), 'Invalid curve, G*N != O');
}
...
SHA256.prototype._update = function _update(msg, start) {
  ...
  assert.deepStrictEqual(this.k.length === W.length, true, new Error('Assertion failed')); // assert(this.k.length === W.length);
...
Hmac.prototype._init = function init(key) {
  ...
  assert.deepStrictEqual(key.length <= this.blockSize, true, new Error('Assertion failed')); // assert(key.length <= this.blockSize);
  // Add padding to key
...
function EC(options) {
  ...
  if (typeof options === 'string') {
    assert.deepStrictEqual(curves.hasOwnProperty(options), true, new Error(`Unknown curve ${options}`)); // assert(curves.hasOwnProperty(options), 'Unknown curve ' + options);
...
EC.prototype.recoverPubKey = function(msg, signature, j, enc) {
  assert.deepStrictEqual((3 & j) === j, true, new Error('The recovery param is more than two bits')); // assert((3 & j) === j, 'The recovery param is more than two bits');
...
function HmacDRBG(options) {
  ...
  assert.deepStrictEqual(entropy.length >= (this.minEntropy / 8), true,
      new Error(`Not enough entropy. Minimum is: ${this.minEntropy} bits`));
...
function Signature(options, enc) {
  ...
  assert.deepStrictEqual(Boolean(options.r && options.s), true, new Error('Signature without r or s')); //assert(options.r && options.s, 'Signature without r or s');
